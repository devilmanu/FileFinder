...
import { Observable, Subject, combineLatest, observable, from } from 'rxjs';
import { FormControl } from '@angular/forms';
import { SharedAnimations } from 'src/app/shared/animations/shared-animations';
import { startWith, debounceTime, switchMap, map, filter, distinctUntilChanged, mergeMap } from 'rxjs/operators';
...
@Component({
  ...
  styleUrls: ['./doc-finder.component.scss'],
  animations: [
    trigger('fadeIn', [
      transition(':enter', [
        style({ opacity: '0' }),
...
export class DocFinderComponent implements OnInit {
  uploadSidebarOpened: boolean = false;
  searchSidebarOpened: boolean = false
  
  page = 1;
  pageSize = 6;

  results$ = new Subject<IFileFindByTextResponse[]>();
   ...
   ngOnInit() {
     this.searchCtrl.valueChanges.pipe(
      ...
      mergeMap((text:string) => {
        return this.fileFinderService.getFileByText(text)
        }
      )
     ).subscribe(o => {
       this.results$.next(o)
  ...
  openSearchSidebar(){
    this.searchSidebarOpened = !this.searchSidebarOpened;
  }

  openUploadSidebar(){
    this.uploadSidebarOpened = !this.uploadSidebarOpened;
  }
